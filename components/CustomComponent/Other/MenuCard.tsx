...
import { Pencil, UserRoundPlus, Settings, Trash2, UsersRound, type LucideIcon } from 'lucide-react'
...
export default function MenuCard({
  ...
  return (
    <div className={`w-[200px] ${backgroundColor} rounded-[10px] py-[15px] flex flex-col gap-[10px]`}>
      {items.reduce((acc: React.ReactNode[], item, index) => {
                ...
                return (
                  ...
                  >
                    <subItem.icon 
                      ...
                      stroke={hoveredItem === subItem.label ? hoverTextColor : style.color} 
                    />
        ...
        if (index < items.length - 1 && index % 2 !== 0) {
          acc.push(<div key={`separator-${index}`} className={`border-t-[1.5px] border-[${separatorColor}]`} />)
        }
        return acc
      }, [])}
    </div>
